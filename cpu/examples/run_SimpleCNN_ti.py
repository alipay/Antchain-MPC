...
from stensorflow.ml.nn.networks.CNN_with_SL import LocalCNN
...
def keras_baseline(train_x, train_y, test_x, test_y, train=True, save_model_path="../output/vgg16_ti_keras.h5"):
    ...
    if train:
        model = tf.keras.models.Sequential([
            ...
            # First Layer
            tf.keras.layers.Conv2D(16, (5, 5), activation='relu', input_shape=(64, 64, 3), use_bias=use_bias),
        ...
        keras_model = tf.keras.models.load_model("../output/cnn_keras.h5")
        ...
        keras_model.save("../output/cnn_keras.h5")
...
def stf_cnn_test(train_x, train_y, test_x, test_y, epochs, load_model=None, save_model_path="../output/complex_CNN.npz",
    ...
    model = LocalCNN(feature=x_train, label=y_train, input_shape=[64, 64, 3], dataset="TinyImageNet")
    ...
    model.predict_to_file(sess, x_test, StfConfig.predict_to_file, pred_batch_num=pred_batch_num,
                          model_file_machine='R', out_prob=False, record_num_ceil_mod_batch_size=0)
    if save_model_path:
