...
//----------------------added-----------------------
template<typename T>
__global__ void sin2kpix(T* a, T* b, size_t m, size_t rows, size_t cols) {
    ...
    if (ROW < rows && COL < cols) {
        ...
        int64_t r = (int64_t) (sin(2 * PI * ((COL+1) * a[ROW]%(1<<m)) / (1<<m)) * (1<<FLOAT_PRECISION));
        b[IDX] = (T) (r+(1UL<<62));
        b[IDX] -= (1UL<<62);
    }
}
...
template<typename T>
__global__ void cos2kpix(T* a, T* b, size_t m, size_t rows, size_t cols) {
      //  (cos 2pix/2^m, sin 2*2*pix/2^m,  sin 2*3*pix/2^m, ...... ) 
    int ROW = blockIdx.y*blockDim.y+threadIdx.y;
    int COL = blockIdx.x*blockDim.x+threadIdx.x;
    ...
    if (ROW < rows && COL < cols) {
        //a[ROW * cols + COL] -= b[rowwise ? COL : ROW];
        size_t IDX=ROW * cols + COL;
        int64_t r = (int64_t) (cos(2 *  PI * ( (COL+1) * a[ROW]%(1<<m)) / (1<<m)) * (1<<FLOAT_PRECISION));
        b[IDX] = (T) (r+(1UL<<62));
        b[IDX] -= (1UL<<62);
    }
}
...
// template<typename T>
// __global__ void sin2kpix(T *a, T *b, size_t m, size_t k, size_t size) {

//     int IDX = blockIdx.x*blockDim.x+threadIdx.x;
    
//     if (IDX < size) {
//         b[IDX] = (T) (sin(2 * k * PI * (a[IDX]%(1<<m)) / (1<<m)) * (1<<FLOAT_PRECISION));
//     } 
// }

// template<typename T>
// __global__ void cos2kpix(T *a, T *b, size_t m, size_t k, size_t size) {

//     int IDX = blockIdx.x*blockDim.x+threadIdx.x;
    
//     if (IDX < size) {
//         b[IDX] = (T) (cos(2 * k * PI * (a[IDX]%(1<<m)) / (1<<m)) * (1<<FLOAT_PRECISION));
//     } 
// }
...
template<typename U>
void sin2kpix(DeviceData<U> &x, DeviceData<U> &y, size_t m, size_t rows, size_t cols) {
        //  (sin 2pix/2^m, sin 2*2*pix/2^m,  sin 2*3*pix/2^m, ...... ) 
    // int cols = x.size();
    // int rows = 1;
    ...
    kernel::sin2kpix<<<blocksPerGrid,threadsPerBlock>>>(
        thrust::raw_pointer_cast(&x.begin()[0]),
        thrust::raw_pointer_cast(&y.begin()[0]),
        m, rows, cols);
...
template<typename U>
void cos2kpix(DeviceData<U> &x, DeviceData<U> &y, size_t m, size_t rows, size_t cols) {
    //  (cos 2pix/2^m, sin 2*2*pix/2^m,  sin 2*3*pix/2^m, ...... ) 
    // int cols = x.size();
    // int rows = 1;
    ...
    kernel::cos2kpix<<<blocksPerGrid,threadsPerBlock>>>(
        thrust::raw_pointer_cast(&x.begin()[0]),
        thrust::raw_pointer_cast(&y.begin()[0]),
        m, rows, cols);
