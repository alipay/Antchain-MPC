...
def inverse_sqrt(x: Union[PrivateTensor, SharedPair], eps=0.0):
    # 1/sqrt(x+eps**2)
    y = x.to_tf_tensor("R")
    y = 1/tf.sqrt(y+eps**2)
    if isinstance(x, SharedPair):
        z = SharedPair(ownerL=x.ownerL, ownerR=x.ownerR, shape=x.shape)
        z.load_from_tf_tensor(y)
    elif isinstance(x, PrivateTensor):
        z = PrivateTensor(owner=x.owner)
        z.load_from_tf_tensor(y)
    else:
        raise Exception("x must be  PrivateTensor or SharedPair")
    return z
