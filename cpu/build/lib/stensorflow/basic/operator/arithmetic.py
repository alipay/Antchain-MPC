...
def matmul3p(x: Union[PrivateTensorBase, SharedPairBase], y: Union[PrivateTensorBase, SharedPairBase, np.ndarray],
             fixed_point=None, prf_flag=None) -> \
        Union[PrivateTensorBase, SharedPairBase]:
    ...
    y_is_private = isinstance(y, PrivateTensorBase)
    y_is_pair = isinstance(y, SharedPairBase)
    y_is_public = isinstance(y, np.ndarray) or isinstance(y, tf.Tensor)
    ...
    if x_is_private and y_is_private:
        ...
        if isinstance(y, tf.Tensor):
            y = tf.cast(y*(1<<x.fixedpoint), 'int64')
        else:
            y = tf.constant(y*(1<<x.fixedpoint), 'int64')
        y = SharedTensorBase(inner_value=y)
        zL = x.xL @ y
        zR = x.xR @ y
