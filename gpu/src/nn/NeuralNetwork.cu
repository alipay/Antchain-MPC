...
template<typename T, template<typename, typename...> typename Share>
void NeuralNetwork<T, Share>::ode_softmax_grad(Share<T> &labels, Share<T> &deltas){
        Share<T> x(deltas.size());
        int nClasses = deltas.size() / MINI_BATCH_SIZE;
    x += *layers[layers.size() - 1]->getActivation();
    softmax(x, deltas, MINI_BATCH_SIZE, nClasses);
    deltas -= labels;
...
void NeuralNetwork<T, Share>::_backward_delta(Share<T> &labels, Share<T> &deltas) {
    ...
    ode_softmax_grad(labels, deltas);
    //deltas.zero();
