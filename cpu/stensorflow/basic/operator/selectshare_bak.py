    ...
    if y is None:
        if x.xL.module is not None and x.xL.module % 2 == 0:
            with tf.device(x.ownerL):
                xL = SharedTensorBase(inner_value=x.xL.inner_value, module=2 * x.xL.module)
            with tf.device(x.ownerR):
                xR = SharedTensorBase(inner_value=x.xR.inner_value, module=2 * x.xR.module)
            ...
            with tf.device(x.ownerL):
                ...
                sxL = SharedTensorBase(inner_value=sxL, module=xL.module//2)

            with tf.device(x.ownerR):
                sxR = two_sx_lift.xR.inner_value - two_sx_lift.xR.inner_value // 2
                sxR = SharedTensorBase(inner_value=sxR, module=xR.module//2)
