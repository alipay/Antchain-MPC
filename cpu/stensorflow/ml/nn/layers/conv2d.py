...
from stensorflow.basic.operator import truncation
...
class Conv2d(Layer):
    ...
    def __init__(self,
        ...
        for father in fathers:
            ...
            kernel = SharedVariablePair(ownerL="L", ownerR="R", shape=self.kernel_shape)
            kernel.load_from_tf_tensor(initializers(self.kernel_shape))
            # print("l330 @ conv2d.py, kernel.fixedpoint=", kernel.fixedpoint)
            self.w += [kernel]
        self.activate = activate

    def __str__(self):
        return "conv2d Layer of output_dim={},".format(self.output_dim)+ "fathers output_dim=" + ", ".join("{}".format(father.output_dim) for father in self.fathers)
    ...
    def func(self, w: List[SharedVariablePair], x: List[Union[PrivateTensor, SharedPair]]):
        ...
        if self.activate is not None:
            if self.activate == 'relu':
                ...
                out = relu(y, self.s)
                out = out.dup_with_precision(x[0].fixedpoint, non_negative=StfConfig.positive_truncation_without_error)
            else:
                raise Exception("unsupported activate function")
        else:
            out = y.dup_with_precision(x[0].fixedpoint)
        return out
    ...
    def pull_back(self,  w: List[SharedPair], x: List[Union[PrivateTensor, SharedPair]],
        ...
        # ploss_px = ploss_px.dup_with_precision(ploss_py.fixedpoint)
        # ploss_pw = ploss_pw.dup_with_precision(ploss_py.fixedpoint)
        ploss_px = ploss_px.dup_with_precision(ploss_py.fixedpoint)
        ploss_pw = ploss_pw.dup_with_precision(ploss_py.fixedpoint)
...
class Conv2dLocal(Layer):
    def __init__(self,
                 ...
                 data_format="NHWC",
                 dilations=[1, 1, 1, 1],
                 prf_flag=None, activate=None, owner="R"):
        if fathers is None:
            raise StfNoneException("fathers")
        if not fathers:
            raise StfCondException("fathers != []", "fathers == []")
