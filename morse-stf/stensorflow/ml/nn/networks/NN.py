...
class NN:
    ...
    def get_train_sgd_op(self, learningRate, l2_regularization):
        """
    ...
    def train_sgd(self, learning_rate, batch_num, l2_regularization, sess):
        learning_rate_list = None
        # 如果传入学习率list，转换为Tensor
        if isinstance(learning_rate, list):
            learning_rate_list = learning_rate
            learning_rate = tf.compat.v1.placeholder(dtype='float64', shape=[])

        train_op = self.get_train_sgd_op(learning_rate, l2_regularization)
        ...
        if learning_rate_list is not None:
            for i in range(batch_num):
                # 每次传入不同的学习率进行梯度下降
                print("batch ", i)
                sess.run(train_op, feed_dict={learning_rate: learning_rate_list[i]})
                if i%10==0:
