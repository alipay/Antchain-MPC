...
from stensorflow.ml.nn.networks.VGG16 import VGG16
...
def keras_baseline(train_x, train_y, test_x, test_y, epochs, save_model_path="../output/vgg16_ti_keras.h5"):
    """
    VGG16 using Keras
    :return:
    """
...
def stf_cnn_test(train_x, train_y, test_x, test_y, epochs, keras_weight=None, learning_rate=0.01, predict_flag=True, batch_size=32, pooling='avg', momentum=0.0):
    ...
    # build model
    model = VGG16(feature=x_train, label=y_train, input_shape=[64, 64, 3], pooling=pooling, dataset='TinyImageNet')
